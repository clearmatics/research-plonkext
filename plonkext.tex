...
\subsection{Simulation extractable NIZKs}
...
\begin{definition}[Simulation-extractable NIZK, \cite{INDOCRYPT:FKMV12}]
  ...
  \emph{simulation-extractable} with \emph{extraction error} $\nu$ if for any
...
We propose a new notion of \emph{updatable simulation extractability} which
...
algorithms: $\algsetup, \algupdate, \algverifysrs$ such that
\begin{description}
\item[$\algsetup$] which takes a relation $\REL$ and outputs an SRS $\srs$, its
  trapdoor $\td$, and proof that $\srs$ has been computed correctly $\rho$.
\item[$\algupdate$] which takes a relation $\REL$, SRS $\srs$ and sequence of
  proofs of updates' correctness $(\rho_i)_{i = 1}^n$ and outputs a new updated
  SRS $\srs'$, update trapdoor $\td$, and proof $\rho'$.
\item[$\algverifysrs$] which takes a relation $\REL$, SRS $\srs$ and sequence of
  proofs of updates' correctness $(\rho_i)_{i = 1}^n$ and outputs $1$ if the
  sequence and SRS are accepted, or $0$ if they are not.
\end{description}

For the sake of simulatability we also define an algorithm $\combinetd$ which
takes trapdoor and updates' trapdoors and combine them into a single final
trapdoor $\td$, cf.~\cref{fig:oraclesrs}.


% Similarly, the simulator is no longer a single algorithm but consists of
% $\algsimsetup, \algsimupdate, \algsimprove$ such that
 % \begin{description}
 % \item[$\algsimsetup$]
 % \item[$\algsimupdate$]
 % \item[$\algsimprove$]
...
\begin{definition}[Updatable soundness~\cite{C:GKMMM18}]
...
\begin{figure}[t]
  	\centering
		\fbox{
		\procedure{$\oraclesrs(\intent, \srs_n, (\td_{j})_{j = 1}^m, (\rho_i)_{i = 1}^{n})$}		
		{
      \pcif \srs \neq \bot \pcthen \pcreturn 0 \\
      \pcif \intent = \setup \pcthen \\
      \pcind (\srs', \td', \rho') \sample \algsetup(\REL)\\
      \pcind Q \gets Q \cup \smallset{\rho'}\\
      \pcind T \gets (T, \td')\\
      \pcind \pcreturn (\srs', \rho')\\
      \pcif \intent = \update \pcthen\\
      \pcind b \gets \algverifysrs(\REL, \srs_n, (\rho_i)_{i = 1}^n)\\
      %\pcind b' \gets \algverifytd(\REL, \srs_n, \td_n)\\
      \pcind \pcif b = 0  \pcthen \pcreturn 0 \\
      \pcind (\srs', \td', \rho') \sample \algupdate(\REL, \srs_n, (\rho)_{i =
        1}^n)
      \\
      \pcind Q \gets Q \cup \smallset{\rho'}\\
      \pcind T \gets (T, (\td_{j})_{j = 1}^m, \td')\\
      \pcind \pcreturn (\srs', \rho') \\
      \pcif \intent = \final \\
      \pcind b \gets \algverifysrs(\REL, \srs_n, (\rho_i)_{i = 1}^n)\\
      \pcind T \gets (T, (\td_{j})_{j = 1}^m)\\
      \pcind \td \gets \combinetd(T)\\
      \pcind b' \gets \algverifytd(\REL, \srs_n, \td)\\
      \pcind \pcif b = 0 \lor b' = 0 \lor Q \cap \smallset{\rho_i}_i = \emptyset
      \pcthen \pcreturn
      0 \\
      \pcind \srs \gets \srs_n\\
      %\pcind \td \gets \combinetd(T)\\
      \pcind \pcreturn (\srs, \td)\\
      \pcelse \pcreturn 0
    }}
...
\end{figure}
  ...
  \begin{definition}[Updatable zero knowledge~\cite{C:GKMMM18}]
    Let $\ps = (\kgen, \prover, \verifier, \simulator)$ be a proof system for
    ...
    \[
      \eps_b = \Pr \left[
        ...
        \, \left| \,
          \begin{aligned}
            % \algsetup \gets \algsimsetup \\
            % \algupdate \gets \algsimupdate\\
...
%         \right]
%       \]
      The oracle $\oracleo_b$ on input $(\inp, \wit)$ returns $\zkproof =
      \simulator(\REL, \srs, \td, \inp)$ if $b = 0$, and $\zkproof =
      \prover(\REL, \srs, \inp, \wit)$ if $b = 1$.
  \end{definition}
  
  Given updatable soundness and subversion zero-knowledge defined, we are ready to define updatable simulation
  extractability
  \begin{definition}[Updatable Special simulation extractability (USSE)]
    \label{def:upsimext}
    Let $\ps = (\kgen, \prover, \verifier, \simulator)$ be a computationally
    special-sound HVZK proof and
    $\ps_\fs = (\kgen_\fs, \prover_\fs, \verifier_\fs, \simulator_\fs)$ be $\ps$
    ...
    the SRS update oracle (cf.~\cref{fig:oraclesrs}) and $\ro$ be a random oracle.
    Let $\adv$ be a $\ppt$ adversary that is given oracle access to
    ...
    if for any such $\adv$ which produces an accepting transcript of $\ps$ with
    probability $\accProb$, i.e.~
    	\[
		\accProb = \Pr \left[
		\begin{aligned}
			& \verifier_\fs(\REL, \srs, \inp_{\advse}, \zkproof_{\advse}) = 1,\\
...
Groth et al.~\cite[Lemma 6]{C:GKMMM18} shown that to assure updatable (knowledge)
soundness of some proof system it is enough to show that it remains (knowledge)
sound if the adversary is allowed to make a single update. Here we show an
equivalent result, stating that to show updatable simulation extractability it is
enough to show that this property holds if the adversary can make a single
update as well. 
\begin{lemma}[Single adversarial update implies full special updatable
  ...
  polynomially many calls to $\oraclesrs$.
\end{lemma}
\begin{proof}
  Similarly to \cite{C:GKMMM18} we assume that $\ps$ is secure against
  advresaries who could perform a single update and build a SSE single-update
  adversary $\bdv$ which outputs an acceptable proof with probability
  $\accProb_\bdv$. By the assumption, there exists an extractor $\ext_\bdv$ who
  from a tree of transcripts obtained from $\bdv$ with probability at least
  $\frkProb_\bdv$. We show how $\bdv$ and its extractor can be used to extract
  the witness from a proof provided by some USSE adversary $\adv$.
  ...
  \item On $\adv$'s queries to $\oraclesrs$, let $k$ be the index of $\adv$'s
    ...
    \item [$(\update, \srs_{n_k}, (\td_j)_{j = 1}^{m_k}, (\rho_i)_{i = 1}^{n_k})$:] Pick
      randomness $r_k$ and run $\algverifysrs(\REL, \srs_{n_k}, (\rho_i)_{i = 1}^{n_k})$
      return $0$ if the verification fails. Otherwise, internally run
      $(\srs', \td', \rho') \gets \algupdate(\REL, \srs_{n_k}, (\rho_i)_{i = 1}^{n_k};
      r_k)$.
    \item [$(\final, \srs_{n_k}, (\td_j)_{j = 1}^{m_k}, (\rho_i)_{i =
        1}^{n_k})$:] Let $N$ be the number of updates requested by $\adv$, 
      \begin{itemize}
      \item Combine the updates requested by $\adv$ into a single update,
        i.e.~run $\algupdate(\REL, \srs_s, \rho_s; \prod_{k = 1}^N r_k)$ and
        get $(\srs_u, \rho_u)$. 
      \item Call real oracle $\oraclesrs$ on $(\final, \srs_u, (\rho_s,
        \rho_u))$. By the construction $\srs_u = \srs_N$. 
      \end{itemize}
    \end{description}
  \item On $\adv$'s queries to the simulator,
    \begin{itemize}
    \item if the SRS has been set.
    \item if the SRS has not been set yet.
    \end{itemize}
  \end{itemize}
\end{proof}
\color{black}
