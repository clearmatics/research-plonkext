...
\changedm{ We propose a new notion of \emph{updatable simulation extractability}
  ...
  it requires. Before we define the aforementioned notion we recall definitions
  of \emph{updatable soundness} and \emph{updatable zero knowledge}. Here, the
  ...
  update the SRS on its own, while here we only assume that the simulator is
  given trapdoors for the generated SRS and updates.

  While an SRS is generated in an updatable manner, there is no single $\kgen$
  algorithm which outputs the string. Instead $\kgen$ compounds of a tuple
  of algorithms: $\algsetup, \algupdate, \algverifysrs$ such that
  \begin{description}
  \item[$\algsetup$] which takes a relation $\REL$ and outputs an SRS
    $\srs$, its trapdoor $\td$, and proof that $\srs$ has been computed correctly $\rho$.
  \item[$\algupdate$] which takes a relation $\REL$, SRS $\srs$ and sequence of
    proofs of updates' correctness $(\rho_i)_{i = 1}^n$ and outputs a new
    updated SRS $\srs'$, update trapdoor $\td$, and proof $\rho'$.
 ...
 % \end{description}
  ...
  \begin{definition}[Updatable soundness~\cite{C:GKMMM18}]
    ...
    $\ppt$ adversary $\adv$, which is given oracle access to an updating oracle $\oraclesrs$
    as defined on \cref{fig:oraclesrs}, there exists an extractor $\ext$ such
    that probability
    \[
      \Pr
      \left[
  ...
  \begin{figure}[t]
      ...
      \pcind (\srs', \td', \rho') \sample \algupdate(\REL, \srs_n, (\rho)_{i = 1}^n)
      \\
      \pcind Q \gets Q \cup \smallset{\rho'}\\
      \pcind T \gets T \cup \smallset{\td'}\\
      \pcind \pcreturn (\srs', \rho') \\
...
\end{figure}
  ...
  \begin{definition}[Updatable zero knowledge~\cite{C:GKMMM18}]
    ...
    $\oracleo_b$, holds $\eps_0(\secpar) - \eps_1(\secpar) \leq \negl$, where
    \[
      \eps_0 = \Pr \left[
        \begin{aligned}
          \adv^{\oracleo_0, \oraclesrs}(\REL) = 1
        \end{aligned}
        \, \left| \,
          \begin{aligned}
...
and
  \[
      ...
      \left[
        ...
        \right]
      \]
      Where the oracles $\oracleo_b$ on input $(\inp, \wit)$ returns $\zkproof =
...
Groth et al.~\cite{C:GKMMM18} shown that to assure updatable (knowledge)
...
update as well. 
\begin{lemma}[Single adversarial update is equivalent to given access to $\oraclesrs$.]
  \end{lemma}
