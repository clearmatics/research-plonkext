...
\begin{proof}
  ...
  \item On $\adv$'s queries to $\oraclesrs$, let $k$ be the index of $\adv$'s
    ...
    \item [$(\update, \srs_{k}, (\td_j)_{j = 1}^{m_k}, (\rho_i)_{i = 1}^{k})$:] Pick
      ...
      $(\srs', \td', \rho') \gets \algupdate(\REL, \srs_{k}, (\rho_i)_{i = 1}^{k};
      r_k)$.
    \item [$(\final, \srs_{n}, (\td_j)_{j = 1}^{m_n}, (\rho_i)_{i = 1}^{n})$:]
      ($n$ is the index of the last $\adv$'s query to
      $\oraclesrs$.)%Let $n$ be the number of updates requested by $\adv$,
      \begin{itemize}
      \item Combine the updates requested by $\adv$ into a single update,
        i.e.~run $\algupdate(\REL, \srs_s, \rho_s; \prod_{k = 1}^n r_k)$ and
        get $(\srs_u, \rho_u)$. 
      \item Call the real oracle $\oraclesrs$ on $(\final, \srs_u, \smallset{\rho_s,
        \rho_u})$. By the construction $\srs_u = \srs_n$. 
      \end{itemize}
    \end{description}
  \item On $\adv$'s queries to the simulator,
    \begin{itemize}
    \item if the SRS has been set, pass the queries to the simulator $\simulator_\fs$;
    \item if the SRS has not been set yet. \michals{11.05}{Problem, since $\bdv$
      run $\setup$, he may not know the whole trapdoor at this point.}
    \end{itemize}
  \end{itemize}
\end{proof}
\color{black}
