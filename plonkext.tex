...
\begin{figure}[t!]
	\begin{pcvstack}[center,boxed]
      ...
      { \pcif \gone{\p{o}(\chi)} \bullet \gtwo{\alpha \chi} + \gone{s - z
        ...
        [\myskip] \rlap{\pcelse \pcreturn 0.} \hphantom{\pcind \pcif \sum_{i =
            1}^{\abs{\vec{z}}} r_i \cdot \gone{\sum_{j = 1}^{t_j} \gamma_i^{j -
              1} c_{i, j} - \sum{j = 1comment{ }^{t_j} s_{i, j}} \bullet
            \gtwo{1} + } }}
		\end{pchstack}
	\end{pcvstack}
...
% \iffalse
% \paragraph{Batching commitments}
% For the sake of efficiency, polynomial commitment openings can be batched. That
% is, commitments $\smallset{c_i}_{i \in I}$ to a set of polynomials
% $\smallset{\p{f}_i}_{i \in I}$, for some $I$, which are evaluated at a point
% $z$ can be opened ``at once'' using a single opening $\op$. The batched opening
% goes as follows:
% \fi
...
% checking a number of equations that depend on the instance, SRS and the
% proof. We call these equations \emph{verification equations} and denote by
% $\vereq_i$, for $i$ being an index of the equation. We require that an
% acceptable proof yields $\vereq_i = 0$.
...
\color{blueish}
\begin{definition}[SRS-updatable $\ur{i}$-protocol]
\label{def:wiur}
Let $\proofsystem$ be a $(2\mu + 1)$-message proof system $\ps = (\kgen,
\prover, \verifier, \simulator)$. Let $\proofsystem_\fs$ be $\proofsystem$ after the
Fiat--Shamir transform, Denote by $a_1, \ldots, a_{\mu}, a_{\mu + 1}$ protocol messages
...
% 	\vphantom{\begin{aligned}
% 	&	\vec{a} = (a_0, b_0, \ldots, a_j, b_j, a_\mu), \vec{a'} = (a'_0, b'_0, \ldots, a'_j,
% 	b'_j a'_\mu) \gets \adv^{\ro, \oraclesrs}(\REL), \vec{a} \neq \vec{a'}, \\
% 	& b_k = b'_k, k \in \range{1, \mu - 1},\\ a_l = a'_l, l \in
% \range{1}{j}, j > i 
% 	\end{aligned}}
% \srs \gets \kgen_\fs(\REL) \right.
...
\begin{definition}[Updatable soundness (US)~\cite{C:GKMMM18}]
  \label{def:us}
...
If $\ps$ from \cref{def:us} is secure against adversaries who are limited to a
single call $\setup$ and a single call $\final$, then $\ps$ is called
\emph{one-time updatable sound} (otUS).
...
\begin{definition}[Updatable computational special soundness (UCSS)]
  \label{def:ucss}
  Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$ be an
  $(2 \mu + 1)$-message proof system for a relation $\REL$. We say that
...
\begin{proof}
  ...
  define $\extt' = \extt$.\michals{12.05}{Check ordering of quantifiers -- the
    extractor should work for all UCSS adversaries, does it? or it works only
    for $\adv$?}  \qed
\end{proof}
...
\begin{proof}[draft]
  ...
  $\srs(\chi)$.  Let $f_{\chi'}$ be a function that given a linear combination
  of vectors' representations in $\srs(\chi)$ produces their representations in
  $\srs(\chi \chi')$.
\end{proof}
