...
\begin{definition}[Updatable $\ur{(k, \vec{a})}$-protocol]
	\label{def:iuru}
	Let $\proofsystem$ be a $(2\mu + 1)$-message public coin proof system
  ...
  \[
	...
	\ldots, \zkproof'_{i}), \\
	%& (\zkproof_{1}, \ldots, \zkproof_{j}) = (\zkproof'_{1}, \ldots, \zkproof'_{j}) \implies \zkproof_{j}.\ch = \zkproof'_{j}.\ch),\\
	& \tilde{\verifier}^{\ro}_{\fs} (\srs, \inp, \zkproof, \vec{z}) =
	\tilde{\verifier}_{\fs} (\srs, \inp, \zkproof', \vec{z'}) = 1  \\
	\end{aligned}
	\,\right|\,
	\begin{aligned}
		& (\srs, \inp, \zkproof, \zkproof', \vec{z}, \vec{z'}) \gets \adv^{\ro, \initU}(1^\secpar) 
	\end{aligned}
	\right] \leq \epsur(\secpar) 
	\]
	Here $\zkproof$ and $\zkproof'$ are two proofs prepared by the adversary which contain the prover's messages.The adversary picks the challenges in $\vec{a}$ on its own. We denote that challenges by $\vec{z}$ (for the proof $\zkproof$), and $\vec{z'}$ (for $\zkproof'$). Verifier $\tilde{\verifier}_{\fs}$ behaves as $\verifier^{\ro}_{\fs}$ except it does not compute the vectors in $\vec{a}$, but takes them from the adversary. If $\epsur(\secpar)$ is negligible, we simply say $\proofsystem$ is $\ur{(k, \vec{a})}$.
\end{definition}
%
