...
Let $\pcalgostyle{Algorithm}$ be an algorithm that (on some input $input$)
returns a $k$-piece output $(out_0, \ldots, out_{k - 1})$. We write
$\pcalgostyle{Algorithm}(input)[i]$ to denote $out_i$.
...
\begin{figure}[t]
      ...
      \pcind b' \gets \algverifytd(\REL, \srs_n, \combinetd((T, (\td_j)_{j = 1}^m))\\
      \pcind \pcif b = 0  \lor b' = 0 \pcthen \pcreturn 0 \\
...
\begin{definition}[Updatable computational special soundness]
  Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$ be an
  $(2 \mu + 1)$-message proof system for a relation $\REL$. We say that
...
\begin{lemma}[One-time updatable computational special soundness (otUCSS) implies
  updatable computational special soundness (UCSS).]
\end{lemma}
\begin{proof}
  ...
  Eventually, the SRS is set and $\adv$ returns an instance $\inp$ and a
  $(n_1, \ldots, n_\mu)$-tree of acceptable transcripts $\tree$. The same
  instance and tree are returned by $\bdv$. From the lemma's assumptions there
  $\extt$ that given $\tree$ outputs the witness corresponding to $\inp$ with
  probability at least $(1 - \epsss)$. Let $\extt'$ be an extractor that
  \michals{12.05}{Issue -- ordering of quantifiers -- the extractor should work
    for all UCSS adversaries, does it? or it works only for $\adv$?}
  \qed
\end{proof}
...
equivalent result, stating that to show updatable special simulation extractability it is
...
\begin{lemma}[Single adversarial update implies full special updatable
  special simulation extractability.]
...
\end{lemma}
\begin{proof}
  % \michals{12.05}{Write down properties for the SRS updates -- that they can be
  %   combined etc.}
  Similarly to \cite{C:GKMMM18} we assume that $\ps$ is secure against
  adversaries who could perform a single update and build a SSE single-update
  adversary $\bdv$ which outputs an acceptable proof with probability
  $\accProb_\bdv$ and internally simulates an USSE adversary $\adv$. By the
  assumption, there exists an extractor $\ext_\bdv$ who from a tree of
  transcripts obtained from $\bdv$ with probability at least $\frkProb_\bdv$. We
  show how $\bdv$ and its extractor can be used to extract the witness from a
  proof provided by $\adv$ which produces an acceptable
  proof with probability $\accProb_\adv$.  The adversary $\bdv$ proceeds as
  follows:
  \begin{itemize}
  \item Run $(\setup, \bot, \bot, \bot)$ and obtain SRS $\srs_s$ and proof
    $\rho_s$.
  ...
  \item On $\adv$'s queries to $\oraclesrs$, let $k$ be the index of $\adv$'s
    ...
    \item [$(\final, \srs_{n}, (\td_j)_{j = 1}^{m_n}, (\rho_i)_{i = 1}^{n})$:]
      ...
      \item Call the real oracle $\oraclesrs$ on $(\final, \srs_u, \smallset{\rho_s,
        \rho_u})$. By the construction $\srs_u = \srs_n$. That is, the SRS finalized
      by the adversary $\adv$ is the same as the SRS finalized by $\bdv$.
      \end{itemize}
    \end{description}
  \item On $\adv$'s queries to the simulator,
    ...
    \item if the SRS has not been set yet: Denote by $\srs_k$ the current SRS,
      ...
      % \michals{11.05}{Problem, since $\bdv$
