...
\begin{theorem}[Simulation-extractable multi-message protocols]
	...
	\emph{updatable $\changedm{k}$-programmable trapdoor-less zero knowledge}, and $(1, \ldots, 1, n_{\changedm{k}}, \ldots, n_\mu)$-\emph{updatable computational special soundness} with security loss $\epscss(\secpar)$. Then $\psfs$ is \emph{updatable simulation-extractable} against any $\ppt$ adversary $\advse$ who return an acceptable proof with probability at least $\accProb$ with 	extraction probability $\extProb$ is at least
...
\begin{proof}	
	...
	Importantly, both proofs are acceptable w.r.t~the unique response property verifier \changedm{as the only programmed challenge is the $k$-th.}
