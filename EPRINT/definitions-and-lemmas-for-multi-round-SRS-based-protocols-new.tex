...
\begin{definition}[Updatable $\ur{k}$-protocol]
	\label{def:iuru}
	Let $\proofsystem$ be a $(2\mu + 1)$-message public coin proof system
  $\ps = (\kgen, \prover, \verifier, \simulator)$ and $\psfs$ its variant after the Fiat--Shamir transformation application. Denote
  by $\zkproof_1, \ldots, \zkproof_{\mu}, \zkproof_{\mu + 1}$ protocol messages
  output by the prover. We write $\zkproof_i.\ch$ to denote a challenge that the prover gets after sending $\zkproof_{i}$. We say that $\proofsystem$ has \emph{unique responses from $i$} with security $\epsur(\secpar)$ if for any $\ppt$ adversary $\advur$:
  \[
	\Pr\left[
	...
	\ldots, \zkproof'_{i}), \\
	%& (\zkproof_{1}, \ldots, \zkproof_{j}) = (\zkproof'_{1}, \ldots, \zkproof'_{j}) \implies \zkproof_{j}.\ch = \zkproof'_{j}.\ch),\\
	& \tilde{\verifier}^{\ro}_{\fs} (\srs, \inp, \zkproof\changedm{, z}) =
	\tilde{\verifier}_{\fs} (\srs, \inp, \zkproof'\changedm{, z'}) = 1  \\
	\end{aligned}
	\,\right|\,
	\begin{aligned}
		& (\srs, \inp, \zkproof, \zkproof'\changedm{, z, z'}) \gets \adv^{\ro, \initU}(1^\secpar) 
	\end{aligned}
	\right] \leq \epsur(\secpar) 
	\]
	\changedm{
	Here $\zkproof$ and $\zkproof'$ are two proofs prepared by the adversary which contain the prover's messages.The adversary picks the $k$-th challenge on its own. We denote that challenge by $z$ (for the proof $\zkproof$), and $z'$ (for $\zkproof'$). Verifier $\tilde{\verifier}_{\fs}$ behaves as $\verifier^{\ro}_{\fs}$ except it does not compute the $k$-th challenge, but takes them from the adversary. If $\epsur(\secpar)$ is negligible, we simply say $\proofsystem$ is $\ur{k}$.}
\end{definition}
%
...
\begin{definition}[($k$-programmable) Trapdoor-Less Simulatable Proof System]
  ...
  If $\epszk(\secpar)$ is negligible, we say $\ps_{\fs}$ is trapdoor-less simulatable. Additionally, we say that $\ps_{\fs}$ is $k$-programmable, if 
	\begin{inparaenum}[(a)]
		...
		\item $\simulator_{\fs}$  programs the random oracle \emph{only} \changedm{at} $k$-th prover's message;
	\end{inparaenum}
  \end{definition}
